package cn.tinbat.andu.crypto.inner.server;
...
public class CryptoInnerServerHandle extends ChannelInboundHandlerAdapter {
    private ServerConfig serverConfig;
    private ChannelFuture cf;
    private String host;
    private int port;

    public CryptoInnerServerHandle(ServerConfig serverConfig) {
    ...
    public void channelRead(final ChannelHandlerContext ctx,final Object msg) throws Exception {
        ...
        if (msg instanceof FullHttpRequest) {// http请求 或 HTTPS建立握手
            ...
            final ServerConfig targetServer = new ServerConfig().host(this.host).port(this.port);
            ChannelFuture cf = bootstrap.connect(serverConfig.getHost(), serverConfig.getPort());
            cf.addListener(new ChannelFutureListener() {
                public void operationComplete(ChannelFuture future) throws Exception {
                    if (future.isSuccess()) {
                        future.channel().writeAndFlush(MessageConfusion.encode(msg,targetServer));
                    } else {
                        ctx.channel().close();
                    }
                }
            ...
            if (cf == null) {
                ...
                final ServerConfig targetServer = new ServerConfig().host(this.host).port(this.port);
                cf.channel().writeAndFlush(MessageConfusion.encode(msg,targetServer));
            }
        }
    }
}
